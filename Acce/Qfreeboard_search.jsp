 ...
 <TABLE border=0 width=95%>
  <TR>
   <TD align=left width=30% valign=bottom>
    [<A href="Qfreeboard_list.jsp">자유 게시판(일반모드)</A>]</TD>
...
<% 
 ...
 try {
  ...
  String sql = "select * from qfreeboard " + cond;
  ...
  if (!(rs.next()))  {
   ...
   for(int j=startrow;j<=endrow;j++) {
    ...
    String clink = "<A href=Qfreeboard_read.jsp?id=" + keyid.elementAt(j) ;
 ...
 if (wheregroup > 1) {
  out.print("[<A href=Qfreeboard_search.jsp?gogroup=1"); 
  out.print("&stype="+ what+"&sval=" + val +">처음</A>]");
  out.print("[<A href=Qfreeboard_search.jsp?gogroup="+priorgroup);
 ...
 if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
   ...
   out.print("[<A href=Qfreeboard_search.jsp?go="+jj);
   out.print("&stype="+ what+"&sval=" + val +">" + jj + "</A>]") ;
   }
  }
 }
 if (wheregroup < totalgroup) {
  out.print("[<A href=Qfreeboard_search.jsp?gogroup="+ nextgroup);
  out.print("&stype="+ what+"&sval=" + val +">다음</A>]");
  out.print("[<A href=Qfreeboard_search.jsp?gogroup="+totalgroup); 

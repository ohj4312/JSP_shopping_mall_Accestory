...
<%
 ...
 if (cont.length()==1) 
  ...
  try {
   ...
   sql = "select max(id) from  qfreeboard";
   ...
   if (step == 1) {
    sql = "select max(replynum) from qfreeboard where masterid="+mid;
    rs=st.executeQuery(sql);
    if (!(rs.next())) 
     rnum=1;
    else 
   ...
   sql= "insert into qfreeboard(id,name,password,email,subject," ;
  ...
  response.sendRedirect("Qfreeboard_list.jsp?go="+request.getParameter("page"));
 %>